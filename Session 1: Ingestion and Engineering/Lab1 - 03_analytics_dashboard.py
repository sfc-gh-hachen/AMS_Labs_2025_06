...
----------------------------------------------------------------------------------
This advanced Streamlit application provides comprehensive business intelligence
and analytics capabilities for talent acquisition data. It connects to multiple
data layers (Silver, Gold, Platinum) to deliver hiring metrics, trend analysis,
geographical insights, and performance dashboards with interactive filtering.

Key Concepts:
  • Multi-layer Data Access: Integration with Silver, Gold, and Platinum data layers
  • Business Intelligence: Advanced analytics and KPI tracking
  • Interactive Filtering: Dynamic data exploration and drill-down capabilities
  • Geospatial Analytics: Location-based hiring insights and mapping
  • Performance Metrics: Time-series analysis and trend identification
----------------------------------------------------------------------------------
"""
...
def load_hiring_metrics():
    """Load customer hiring metrics - now using Silver layer for consistency"""
    query = f"""
    WITH consistent_metrics AS (
        SELECT 
            SNAPSHOT_YEAR,
            SNAPSHOT_QUARTER,
            SNAPSHOT_MONTH,
            ...
            -- Count unique applications to avoid double counting
            COUNT(DISTINCT UNIQUE_APPLICATION_ID) AS TOTAL_APPLICATIONS,
            ...
            -- Outcome counts
            COUNT(CASE WHEN APPLICATION_OUTCOME_CATEGORY = 'Applied' THEN 1 END) AS APPLIED_COUNT,
            COUNT(CASE WHEN APPLICATION_OUTCOME_CATEGORY = 'In Process' THEN 1 END) AS IN_PROCESS_COUNT,
            COUNT(CASE WHEN APPLICATION_OUTCOME_CATEGORY = 'Offered' THEN 1 END) AS OFFERED_COUNT,
            COUNT(CASE WHEN APPLICATION_OUTCOME_CATEGORY = 'Hired' THEN 1 END) AS HIRED_COUNT,
            COUNT(CASE WHEN APPLICATION_OUTCOME_CATEGORY = 'Declined' THEN 1 END) AS DECLINED_COUNT,
            COUNT(CASE WHEN APPLICATION_OUTCOME_CATEGORY = 'Withdrawn' THEN 1 END) AS WITHDRAWN_COUNT,
            
            -- Success rate calculation
            CASE WHEN COUNT(DISTINCT UNIQUE_APPLICATION_ID) > 0 THEN
                ROUND((COUNT(CASE WHEN APPLICATION_OUTCOME_CATEGORY = 'Hired' THEN 1 END) * 100.0) / COUNT(DISTINCT UNIQUE_APPLICATION_ID), 2)
        ...
        GROUP BY 
            ...
            JOB_FUNCTION_CATEGORY, APPLICATION_OUTCOME_CATEGORY, SOURCING_CHANNEL_TYPE_CLEAN, SOURCING_CHANNEL_NAME_CLEAN,
            HIRE_TIME_BUCKET, REJECTION_TIME_BUCKET
    )
    SELECT * FROM consistent_metrics
...
def load_application_summary():
    """Load application summary data for funnel analysis - simplified version"""
    query = f"""
    SELECT 
        COUNT(DISTINCT UNIQUE_APPLICATION_ID) as TOTAL_APPLICATIONS,
        COUNT(CASE WHEN APPLICATION_OUTCOME_CATEGORY = 'Applied' THEN 1 END) as APPLIED_COUNT,
    ...
    FROM {DATABASE}.{SCHEMA}.SILVER_APPLICATIONS_ENRICHED
    WHERE SNAPSHOT_DATE >= CURRENT_DATE() - 365
    GROUP BY GEOGRAPHIC_REGION, JOB_FUNCTION_CATEGORY, SOURCING_CHANNEL_TYPE_CLEAN
    ...
    try:
        ...
        st.error(f"Error loading application summary: {str(e)}")
        return pd.DataFrame()
...
def validate_data_consistency(hiring_metrics, application_summary):
    """Validate consistency between different data sources"""
    warnings = []
    
    if not hiring_metrics.empty and not application_summary.empty:
        # Compare total applications between sources (both now use Silver layer)
        exec_overview_total = hiring_metrics['TOTAL_APPLICATIONS'].sum()
        funnel_total = application_summary['TOTAL_APPLICATIONS'].sum()
        
        # Since both use same Silver layer, they should be very close (within 1%)
        variance_threshold = 0.01
        if abs(exec_overview_total - funnel_total) / max(exec_overview_total, funnel_total) > variance_threshold:
            warnings.append(f"⚠️ Minor Data Variance: Executive Overview shows {exec_overview_total:,}, Funnel shows {funnel_total:,}")
        else:
            st.success(f"✅ Data Consistency Verified: Both sections show consistent totals (~{exec_overview_total:,} applications)")
        
        if len(warnings) > 0:
            st.warning("Data Consistency Issues Detected:")
            for warning in warnings:
                st.write(warning)
    elif not hiring_metrics.empty:
        exec_total = hiring_metrics['TOTAL_APPLICATIONS'].sum()
        st.info(f"📊 Single Data Source: Executive Overview shows {exec_total:,} applications from Silver layer")
...
def main():
    ...
    with st.spinner("Loading customer analytics data..."):
        ...
        application_summary = load_application_summary()
    
    if hiring_metrics.empty and trends_data.empty and application_summary.empty:
    ...
    validate_data_consistency(hiring_metrics, application_summary)
    ...
    # Region filter - use combined data sources for complete filter options
    all_regions = set()
    if not hiring_metrics.empty:
        all_regions.update(hiring_metrics['GEOGRAPHIC_REGION'].dropna().unique())
    if not application_summary.empty:
        all_regions.update(application_summary['GEOGRAPHIC_REGION'].dropna().unique())
    
    regions = ['All Regions'] + sorted(list(all_regions))
    selected_region = st.sidebar.selectbox("🌍 Geographic Region", regions)
    
    # Job function filter
    all_functions = set()
    if not hiring_metrics.empty:
        all_functions.update(hiring_metrics['JOB_FUNCTION_CATEGORY'].dropna().unique())
    if not application_summary.empty:
        all_functions.update(application_summary['JOB_FUNCTION_CATEGORY'].dropna().unique())
    
    functions = ['All Functions'] + sorted(list(all_functions))
    selected_function = st.sidebar.selectbox("💼 Job Function", functions)
    
    # Sourcing channel filter
    all_channels = set()
    if not hiring_metrics.empty:
        all_channels.update(hiring_metrics['SOURCING_CHANNEL_TYPE_CLEAN'].dropna().unique())
    if not application_summary.empty:
        all_channels.update(application_summary['SOURCING_CHANNEL_TYPE_CLEAN'].dropna().unique())
    
    channels = ['All Channels'] + sorted(list(all_channels))
    ...
    filtered_summary = application_summary.copy()
    
    if not hiring_metrics.empty or not application_summary.empty:
        if selected_region != 'All Regions':
            ...
            if not application_summary.empty:
                filtered_summary = filtered_summary[filtered_summary['GEOGRAPHIC_REGION'] == selected_region]
        ...
        if selected_function != 'All Functions':
            ...
            if not application_summary.empty:
                filtered_summary = filtered_summary[filtered_summary['JOB_FUNCTION_CATEGORY'] == selected_function]
        ...
        if selected_channel != 'All Channels':
            ...
            if not application_summary.empty:
                filtered_summary = filtered_summary[filtered_summary['SOURCING_CHANNEL_TYPE_CLEAN'] == selected_channel]
    ...
    with tab2:
        show_application_funnel_flow(filtered_hiring, filtered_detailed, filtered_summary)
...
def show_executive_overview(hiring_metrics, trends_data):
    ...
    if not hiring_metrics.empty:
        ...
        st.info(f"📊 Data Source: Silver Layer Consistent Aggregation (Fixed Double-Counting) | Total Records: {len(hiring_metrics)} | Date Range: {hiring_metrics['SNAPSHOT_YEAR'].min()}-{hiring_metrics['SNAPSHOT_YEAR'].max()}")
        
        with st.expander("🔧 Data Consistency Fix Details"):
            st.markdown("""
            **Previous Issue**: Executive Overview used Gold layer data which grouped by multiple dimensions, 
            causing the same application to be counted multiple times (e.g., once for each region/job function combination).
            
            **Solution Applied**: Both Executive Overview and Sankey diagram now use Silver layer data with 
            `COUNT(DISTINCT UNIQUE_APPLICATION_ID)` to ensure each application is counted exactly once.
            
            **Result**: Consistent application totals across all dashboard sections.
            """)
...
def show_application_funnel_flow(hiring_metrics, detailed_data, application_summary):
    ...
    # Use application_summary for consistent funnel data, fall back to detailed_data if needed
    primary_data = application_summary if not application_summary.empty else detailed_data
    
    if not primary_data.empty:
        # Calculate funnel flow data using consistent summary data
        flow_data = calculate_funnel_flow_data(primary_data, use_summary_data=not application_summary.empty)
        ...
        if flow_data:
            ...
            # Add data source info for debugging
            if not application_summary.empty:
                st.info(f"🔄 Funnel Data Source: Silver Layer Aggregated Summary | Filtered Records: {len(application_summary)} | Total Apps: {application_summary['TOTAL_APPLICATIONS'].sum():,}")
                st.success("✅ **Data Consistency Fixed**: Both Executive Overview and Sankey diagram now use the same Silver layer source with unique application counting to eliminate double-counting issues.")
            else:
                st.info(f"🔄 Funnel Data Source: Silver Layer Detailed Data | Records: {len(detailed_data):,}")
            
            # Add process flow insights below the diagram
            # Use consistent data source for insights
            if not application_summary.empty:
                # Use aggregated summary data
                total_applications = application_summary['TOTAL_APPLICATIONS'].sum()
                applied_count = application_summary['APPLIED_COUNT'].sum()
                in_process_count = application_summary['IN_PROCESS_COUNT'].sum()
                offered_count = application_summary['OFFERED_COUNT'].sum()
                hired_count = application_summary['HIRED_COUNT'].sum()
                declined_count = application_summary['DECLINED_COUNT'].sum()
                withdrawn_count = application_summary['WITHDRAWN_COUNT'].sum()
            else:
                # Fall back to detailed data counts
            ...
            # Create stage breakdown using consistent data
            if not application_summary.empty:
                # Use summary data for consistent breakdown
                stage_breakdown = pd.DataFrame({
                    'Stage': ['Applied', 'In Process', 'Offered', 'Hired', 'Declined', 'Withdrawn'],
                    'Count': [applied_count, in_process_count, offered_count, hired_count, declined_count, withdrawn_count]
                })
                stage_breakdown = stage_breakdown[stage_breakdown['Count'] > 0]  # Remove zero counts
            else:
                # Fall back to detailed data
...
def calculate_funnel_flow_data(data, use_summary_data=False):
    """Calculate data for Sankey diagram showing realistic recruitment process flow"""
    try:
        if use_summary_data:
            # Use pre-aggregated summary data
            total_apps = data['TOTAL_APPLICATIONS'].sum()
            applied_count = data['APPLIED_COUNT'].sum()
            in_process_count = data['IN_PROCESS_COUNT'].sum()
            offered_count = data['OFFERED_COUNT'].sum()
            hired_count = data['HIRED_COUNT'].sum()
            declined_count = data['DECLINED_COUNT'].sum()
            withdrawn_count = data['WITHDRAWN_COUNT'].sum()
        else:
            # Get actual counts from detailed data
            stage_counts = data['APPLICATION_OUTCOME_CATEGORY'].value_counts().to_dict()
            total_apps = len(data)
